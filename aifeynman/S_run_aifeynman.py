...
results_path = 'results_' + date_string


def run_AI_all(pathdir, filename, BF_try_time=60, BF_ops_file_type="14ops", polyfit_deg=4, NN_epochs=4000, device='cpu', PA=PA):
    ...
    PA = get_squared(pathdir, f"{results_path}/mystery_world_squared/", filename, BF_try_time, BF_ops_file_type, PA, polyfit_deg)

    # Run bf and polyfit on modified output
    PA = get_acos(pathdir, f"{results_path}/mystery_world_acos/", filename, BF_try_time, BF_ops_file_type, PA, polyfit_deg)
    PA = get_asin(pathdir, f"{results_path}/mystery_world_asin/", filename, BF_try_time, BF_ops_file_type, PA, polyfit_deg)
    PA = get_atan(pathdir, f"{results_path}/mystery_world_atan/", filename, BF_try_time, BF_ops_file_type, PA, polyfit_deg)
    PA = get_cos( pathdir, f"{results_path}/mystery_world_cos/", filename, BF_try_time, BF_ops_file_type, PA, polyfit_deg)
    PA = get_exp( pathdir, f"{results_path}/mystery_world_exp/", filename, BF_try_time, BF_ops_file_type, PA, polyfit_deg)
    PA = get_inverse(pathdir,f"{results_path}/mystery_world_inverse/", filename,BF_try_time, BF_ops_file_type, PA, polyfit_deg)
    PA = get_log(pathdir, f"{results_path}/mystery_world_log/", filename, BF_try_time, BF_ops_file_type, PA, polyfit_deg)
    PA = get_sin(pathdir, f"{results_path}/mystery_world_sin/", filename, BF_try_time, BF_ops_file_type, PA, polyfit_deg)
    PA = get_sqrt(pathdir, f"{results_path}/mystery_world_sqrt/", filename, BF_try_time, BF_ops_file_type, PA, polyfit_deg)
    PA = get_squared(pathdir, f"{results_path}/mystery_world_squared/", filename, BF_try_time, BF_ops_file_type, PA, polyfit_deg)
    PA = get_tan(pathdir, f"{results_path}/mystery_world_tan/", filename, BF_try_time, BF_ops_file_type, PA, polyfit_deg)
...
#############################################################################################################################
    ...
    if len(data[0])<3:
        ...
        pass
    elif os.path.exists(f"{results_path}/NN_trained_models/models/" + filename + ".h5"):
        print("NN already trained \n")
        print("NN loss: ", NN_eval(pathdir,filename, device, results_path)[0], "\n")
        model_feynman = NN_eval(pathdir, filename, device, results_path)[1]
    elif os.path.exists(f"{results_path}/NN_trained_models/models/" + filename + "_pretrained.h5"):
        print("Found pretrained NN \n")
        model_feynman = NN_train(pathdir, filename, NN_epochs, device, lrs=1e-3, N_red_lr=3, pretrained_path=f"{results_path}/NN_trained_models/models/" + filename + "_pretrained.h5", results_path=results_path)
        print("NN loss after training: ", NN_eval(pathdir, filename, device, results_path), "\n")
    else:
        print("Training a NN on the data... \n")
        model_feynman = NN_train(pathdir, filename,NN_epochs, device, results_path=results_path)
        print("NN loss: ", NN_eval(pathdir, filename, device, results_path), "\n")


    # Check which symmetry/separability is the best
    # Symmetries
    print("Checking for symmetries...")
    symmetry_minus_result = check_translational_symmetry_minus(pathdir, filename, device, results_path)
    symmetry_divide_result = check_translational_symmetry_divide(pathdir, filename, device, results_path)
    symmetry_multiply_result = check_translational_symmetry_multiply(pathdir, filename, device, results_path)
    symmetry_plus_result = check_translational_symmetry_plus(pathdir, filename, device, results_path)
    print("")

    print("Checking for separabilities...")
    # Separabilities
    separability_plus_result = check_separability_plus(pathdir, filename, device, results_path)
    separability_multiply_result = check_separability_multiply(pathdir, filename, device, results_path)
    ...
    try:
        succ_grad = evaluate_derivatives(pathdir, filename, model_feynman, device)
    except:
        succ_grad = 0
    ...
    if succ_grad == 1:
        #try:
        for qqqq in range(1):
            brute_force_comp(f"{results_path}/","gradients_comp_%s.txt" %filename,600,"14ops.txt")
            bf_all_output = np.loadtxt("results_comp.dat", dtype="str")
            for bf_i in range(len(bf_all_output)):
                idx_comp_temp = 0
                try:
                    express = bf_all_output[:,1][bf_i]
                    idx_comp_temp, eqq, new_mu, new_sigma = check_compositionality(pathdir, filename, model_feynman, device, express, mu, sigma, nu=10)
                    if idx_comp_temp==1:
                        idx_comp = 1
                        math_eq_comp = eqq
                        mu = new_mu
    ...
    for kiiii in range(1):
        if len(data[0])>3:
            ...
            decomp_idx = identify_decompositions(pathdir, filename, model_feynman, device, results_path)
            brute_force_gen_sym(f"{results_path}/","gradients_gen_sym_%s" %filename, 600, "14ops.txt")
            ...
            for bf_i in range(len(bf_all_output)):
                ...
                try:
                    express = bf_all_output[:,1][bf_i]
                    idx_gen_sym_temp, eqq, new_mu, new_sigma = check_gen_sym(pathdir, filename, model_feynman, device, decomp_idx, express, mu, sigma, nu=10)
    ...
    if idx_min == 0:
        ...
        new_pathdir, new_filename = do_translational_symmetry_plus(pathdir,filename,symmetry_plus_result[1],symmetry_plus_result[2], device, results_path)
        PA1_ = ParetoSet()
        PA1 = run_AI_all(new_pathdir,new_filename,BF_try_time,BF_ops_file_type, polyfit_deg, NN_epochs, device, PA1_)
        ...
        new_pathdir, new_filename = do_translational_symmetry_minus(pathdir,filename,symmetry_minus_result[1],symmetry_minus_result[2], device, results_path)
        PA1_ = ParetoSet()
        PA1 = run_AI_all(new_pathdir,new_filename,BF_try_time,BF_ops_file_type, polyfit_deg, NN_epochs, device, PA1_)
        ...
        new_pathdir, new_filename = do_translational_symmetry_multiply(pathdir,filename,symmetry_multiply_result[1],symmetry_multiply_result[2], device, results_path)
        PA1_ = ParetoSet()
        PA1 = run_AI_all(new_pathdir,new_filename,BF_try_time,BF_ops_file_type, polyfit_deg, NN_epochs, device, PA1_)
        ...
        new_pathdir, new_filename = do_translational_symmetry_divide(pathdir,filename,symmetry_divide_result[1],symmetry_divide_result[2], device, results_path)
        PA1_ = ParetoSet()
        PA1 = run_AI_all(new_pathdir,new_filename,BF_try_time,BF_ops_file_type, polyfit_deg, NN_epochs, device, PA1_)
        ...
        new_pathdir1, new_filename1, new_pathdir2, new_filename2,  = do_separability_plus(pathdir,filename,separability_plus_result[1],separability_plus_result[2], device, results_path)
        PA1_ = ParetoSet()
        PA1 = run_AI_all(new_pathdir1,new_filename1,BF_try_time,BF_ops_file_type, polyfit_deg, NN_epochs, device, PA1_)
        PA2_ = ParetoSet()
        PA2 = run_AI_all(new_pathdir2,new_filename2,BF_try_time,BF_ops_file_type, polyfit_deg, NN_epochs, device, PA2_)
        ...
        new_pathdir1, new_filename1, new_pathdir2, new_filename2,  = do_separability_multiply(pathdir,filename,separability_multiply_result[1],separability_multiply_result[2], device, results_path)
        PA1_ = ParetoSet()
        PA1 = run_AI_all(new_pathdir1,new_filename1,BF_try_time,BF_ops_file_type, polyfit_deg, NN_epochs, device, PA1_)
        PA2_ = ParetoSet()
        PA2 = run_AI_all(new_pathdir2,new_filename2,BF_try_time,BF_ops_file_type, polyfit_deg, NN_epochs, device, PA2_)
        ...
        new_pathdir, new_filename = do_gen_sym(pathdir, filename, decomp_idx, math_eq_gen_sym, results_path)
        PA1_ = ParetoSet()
        PA1 = run_AI_all(new_pathdir,new_filename,BF_try_time,BF_ops_file_type, polyfit_deg, NN_epochs, device, PA1_)
...
def run_aifeynman(pathdir, filename, BF_try_time, BF_ops_file_type, polyfit_deg=4, NN_epochs=4000, gpu=-1, vars_name=[], test_percentage=20):
    ...
    PA = run_AI_all(pathdir,filename+"_train", BF_try_time, BF_ops_file_type, polyfit_deg, NN_epochs, device, PA=PA)
    ...
    for i in range(len(PA_list)):
        try:
            PA = add_bf_on_numbers_on_pareto(pathdir,filename,PA,PA_list[i][-1], results_path)
        except:
            continue
    PA_list = PA.get_pareto_points()
    '''

    np.savetxt(f"{results_path}/solution_before_snap_%s.txt" %filename,PA_list,fmt="%s")
    ...
    PA_list = PA.get_pareto_points()
    np.savetxt(f"{results_path}/solution_first_snap_%s.txt" %filename,PA_list,fmt="%s")
    ...
    np.savetxt(f"{results_path}/solution_%s" %filename_orig,save_data,fmt="%s")
    try:
    ...
    for dfile in dat_files:
        shutil.move(dfile, f'{results_path}/')
